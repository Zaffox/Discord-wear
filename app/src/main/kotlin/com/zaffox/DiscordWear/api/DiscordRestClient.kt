...
class DiscordRestClient(private val token: String) {
    ...
    suspend fun getGuildChannels(
        ...
        // Fetch member + roles so we can compute real VIEW_CHANNEL permissions.
        // /guilds/{id}/members/@me works with user tokens; /guilds/{id} returns roles.
        // If either call fails we fall back to showing all channels rather than hiding everything.
        val member = getGuildMember(guildId).getOrNull()
        val roles  = getGuildRoles(guildId).getOrNull()

        fun canView(channel: Channel): Boolean {
            if (member == null || roles == null) return true // no data → show
