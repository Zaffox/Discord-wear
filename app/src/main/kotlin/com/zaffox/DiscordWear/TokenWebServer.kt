...
/**
 * A minimal HTTP server that serves a token-entry page on port 8080.
 * Useful for entering your Discord token from a browser on your phone or PC
 * without typing on the watch.
 *
 * Usage:
 *   val server = TokenWebServer(port = 8080) { token -> /* save token */ }
 *   server.start()
 *   // later:
 *   server.stop()
 */
class TokenWebServer(
    ...
    /** Returns the local IP addresses the server is reachable at. */
    ...
    private suspend fun handleClient(socket: Socket) = withContext(Dispatchers.IO) {
        try {
            ...
            if (isPost && contentLength > 0) {
                ...
                // Parse token from form body: token=xxxxx
            ...
            if (!submittedToken.isNullOrBlank()) {
                // Respond with success page, then call back
...
<div class="card">
  ...
  <div class="warn">⚠️ Never share your token. Only enter it on your own device.</div>
  <form method="POST" action="/">
    <input type="password" name="token" placeholder="Paste your Discord token…" autocomplete="off" required>
    <button type="submit">Save Token to Watch</button>
  </form>
</div>
</body>
</html>"""
...
<div class="card">
  <div class="icon">✅</div>
  <h1>Token Saved!</h1>
  <p>Your Discord token has been saved to the watch. You can now close this page and use DiscordWear.</p>
</div>
</body>
</html>"""
}
